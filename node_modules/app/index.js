...
if (window.localStorage && window.localStorage.removeItem) {
  ...
  if (storageVersion < app.version) {
    var valsToKeep = ('appUrl locale lastUsedEmail').split(' ');
    var backup = {};
...
app.on('event-logout', function (eid) { // cleanup local storage
  ...
  if (!event || event.email || event.password) {
    console.log('clear event storage', event.id);
    window.localStorage.removeItem('eventData'+ event.id);
    window.localStorage.removeItem('eventStorageTime'+ event.id);
    window.localStorage.removeItem('cart'+ event.id);
    window.localStorage.removeItem('favorites'+ event.id);
    window.localStorage.removeItem('hides'+ event.id);
    window.localStorage.removeItem('buyPreset'+ event.id);
  }
});
