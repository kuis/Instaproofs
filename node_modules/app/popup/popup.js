...
module.exports = new (BaseView.extend({
  viewType: 'popup',
  template: require('app/templates').popup.popup,
  ...
  props: {
    ...
    scrollable: ['boolean', true, false], // whether scrollbar may be needed
  },
  ...
  show: function (ops) {
    ...
    if (!this.customLayout) {
      if (!ops.image) this.maxWidth -= 100;
      if (ops.content.length < 150) this.maxWidth -= 80;
    }
  ...
  resize: function (ops) {
    var width, height, top, left, targetRect, contentWidth, contentHeight,
    ...
    if (this.minWidth) {
      ...
      if (ops) {
        ...
        contentWidth = this.scrollEl.scrollWidth;
        if (contentWidth < maxWidth) {
          this.maxWidth = maxWidth = contentWidth;
        }
      }
    }
    ...
    if (this.targetEl) {
      ...
      if (app.state.width <= maxWidth + 80) { // tight viewport
        width = app.state.width > (maxWidth+30) ? maxWidth 
                                                : app.state.width - 30;
        left = Math.round((app.state.width - width) / 2);
        pointerSide = 'none';
      }
      else if (toLeft >= maxWidth || toRight >= maxWidth) { // enough on a side 
        if (toRight >= toLeft) {
          width = toRight > maxWidth ? maxWidth : toRight;
          left = Math.round(targetRect.left + targetRect.width + 10);
          pointerSide = 'left';
        }
        else {
          width = toLeft > maxWidth ? maxWidth : toLeft;
      ...
      width = app.state.width > (maxWidth+30) ? maxWidth : app.state.width - 30;
      left = Math.round((app.state.width - width) / 2);
      pointerSide = 'none';
    }
    ...
    contentHeight = this.contentWrapper.offsetHeight + 1;
    contentWidth = this.contentWrapper.offsetWidth + 1;
    
    this.width = this.scrollEl.scrollWidth;
