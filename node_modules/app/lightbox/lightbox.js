...
module.exports = BaseView.extend(SlideshowMixin, TouchMixin, {
  ...
  inputIntervalId: null, // interval ID for user inactivity 
  displayed: false, // whether the lightbox is currently displayed
  lastUserActivity: 0, // timestamp
  ...
  initialize: function () {
    ...
    this.namesDisplayed = !!app.config.display_photo_names;
    
    this.bindMethods('back', 'forward', 'close', 'checkUserActivity', 
                     'slideshowCallback','onMouseRest');
    
    if (ua.mobile) this.mouseRestSpeed = 2000;
    
    setInterval(this.checkUserActivity, 300);
    ...
    this.cacheElements({
      ...
      cartBadgeEl: '.cart.top-right-icon span.badge',
    ...
    this.listenTo(app, 'offline', function () {
  ...
  checkUserActivity: function () {
    var now = Date.now();
    if (now - this.lastUserActivity >= this.mouseRestSpeed) {
      // this.mouseRest = true;
  ...
  reset: function (ops) {
    ...
    this.listenTo(this.event.cart, 'change:length', this.updateLength);
    ...
    this.listenTo(this.event.favorites, 'change:mtime', function () {
      ...
      if (this.favoritesEmpty && 
          this.collection === this.event.favorites.photos) {
        app.navigate('events/'+ this.event.id);
        return;
      }
  ...
  toggleImageNames: function (e) {
    ...
    app.config.display_photo_names = this.namesDisplayed ? true : false;
  },
  ...
  updateCartIcon: function () {
    ...
    this.cartBadgeEl.classList.toggle('hidden', this.event && this.event.cart.is_empty);
  },
  updateLength: function () {
    this.cartLength = this.event.cart.length;
  },
