...
var events = [];
// photos are separated as they are not included for products listing
var photos = {};
...
var prodTitles = ['Bride & Groom', 'Family Formals', 'Rose Ceremony',
...
function createEvent(index) {
  ...
  // var prodsNum = (prodTitles.length + index - 2) % (prodTitles.length-1) + 2;
  var prodsNum = (prodTitles.length + index - 2) % (prodTitles.length-1) + 1;
  for (var i = 0; i < prodsNum; i++) {
    addProduct(ev, i);
  }
...
function addProduct(ev, index) {
  ...
  var music = [];
  if (ev.id % 3 === 2) { // every 2nd event of 3 has different list per prod
    for (i = 0; i < tracksNum; i++) {
      music.push(tracks[(prodID*tracksNum+i) % tracks.length]);
    }
  }
  else if (ev.id % 3 !== 0) { // 2 of 3 products have same list for all prods 
    for (i = 0; i < tracksNum; i++) {
  ...
  ev.products[index] = {
    id: prodID++,
    name: prodTitles[index],
    img_url: "/img/pics/img" + (1 + (prodID % 19)) +'.jpg?' + (uid++),
    img_count: photosEnd - photosStart + 1,
    music: music,
    // music: ev.id % 3 === 2 ? [tracks[ev.id % tracks.length]] :
    //        ev.id % 3 !== 0 ? [tracks[prodID % tracks.length]] : '',
  };
  ...
  for (i = photosStart; i <= photosEnd; i++) {
    ...
    photos['prod'+ ev.products[index].id].push({
      ...
      img_url: '/img/pics/products/1/img'+ img +'.jpg?'+ (uid++),
