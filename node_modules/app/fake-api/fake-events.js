...
var productTitles = ['2 Photo Album', '5 Photo Album', '10 Photo Album',
    '15 Photo Album', '20 Photo Album', '25 Photo Album', 
    '30 Photo Album', '40 Photo Album', '50 Photo Album'];
var tracks = [
...
function createEvent(index) {
  ...
  var catsNum = (catTitles.length + index - 2) % (catTitles.length-1) + 1;
  for (var i = 0; i < catsNum; i++) {
    addCategory(ev, i);
  }
  
  var productsNum = (productTitles.length + index - 2) % (productTitles.length-1);
...
function addCategory(ev, index) {
  ...
  var tracksNum = (ev.id * 3) % (tracks.length+1);
  var music = [];
  if (ev.id % 3 === 2) { // every 2nd event of 3 has different list per cat
    for (i = 0; i < tracksNum; i++) {
      music.push(tracks[(catID*tracksNum+i) % tracks.length]);
    }
  }
  else if (ev.id % 3 !== 0) { // 2 of 3 events have same list for all cats 
    for (i = 0; i < tracksNum; i++) {
      music.push(tracks[(ev.id+i) % tracks.length]);
    }
  }
  ...
  for (i = photosStart; i <= photosEnd; i++) {
    img = (70 + i) % 71 + 1;
    ...
    photos['cat'+ ev.categories[index].id].push({
      ...
      width: landscape ? 1280 : 941,
      height: landscape ? 941 : 1280,
      original_width: (landscape ? 1280 : 941) * 4, // assume original is ~30mpx
      original_height: (landscape ? 941 : 1280) * 4,
      img_url: '/img/pics/events/1/img'+ img +'.jpg?'+ (uid++),
      download_url: img % 4 === 0 ? '/img/pics/events/1/img'+ img +'.jpg': '',
...
function addProduct(ev, index) {
  var img, landscape;
  var photosStart = (71 + ev.id * prodID * 2) % 71 + 1;
  var photosNum = (40 + ev.id * index * 30) % 250;
  var photosEnd = photosStart + ((photosNum < 20) ? 20 : photosNum);
  var img_url = "/img/pics/img" + (1+(prodID % 19)) +'.jpg?' + (uid++);
  ...
  img = (70 + index) % 71 + 1;
  landscape = landscapePhotoRE.test(img);
  photos['prod'+ ev.products[index].id].push({
    id: photoID++,
    name: productTitles[index],
    width: landscape ? 1280 : 941,
    height: landscape ? 941 : 1280,
    img_url: img_url,
    download_url: img % 4 === 0 ? '/img/pics/products/1/img'+ img +'.jpg': '',
  });
  prodID++;
}
