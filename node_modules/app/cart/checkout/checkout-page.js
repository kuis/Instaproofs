...
module.exports = CartPage.extend(PaypalMixin, {
  ...
  checkoutPaypal: function () {
    ...
    // Verifies cart total amount
    this.verifyCartTotal(cid, {
      event_id: this.model.event.id,
      shipping_type: this.model.shipping_type,
      discount_code: this.model.discountCode.discount_code
      ...
      if (response.data.data.total !== self.model.total) {
        alert('Total amount does not match with server');
        return;
      }
      ...
      self.startTransaction(cid, {
        ...
        app.storage['paypal_token' + self.model.event.id] = response.data.data.token;
