"use strict";

var Page = require('app/base/base-page');
var CartPage = require('app/cart/cart-page');
var Countries = require('app/util/countries');
var OrderModel = require('./order-model');
var States = require('app/util/states');
var Summary = require('../summary/cart-summary');
var PaypalMixin = require('./paypal-mixin');

var app = require('app');
var assign = require('lodash.assign');
var templates = require('app/templates');
var xhr = require('xhr');

module.exports = CartPage.extend(PaypalMixin, {
  viewType: 'checkout-page',
  dictionary: 'cart.checkout',
  pageTitle: 'Checkout',
  pageType: 'event-checkout',

  template: templates.cart.checkout['checkout-page'],

  props: {
    eventId: ['number', true, 0],
    summaryPosition: ['string', true, 'static'],
    separateBilling: ['boolean', true, false],
    saveShipping: ['boolean', true, false],

    shippingCountrySelected: ['boolean', true, false],
    billingCountrySelected: ['boolean', true, false],
    requiresDisplayed: ['boolean', true, false],

    cachedShipping: ['object', true],
    cachedBilling: ['object', true]
  },

  derived: {
    // isTotalEmpty: {
    //   deps: ['model.total'],
    //   fn: function () {
    //     return this.model.total === 0;
    //   }
    // }
  },

  events: {
    'click .billing-as-shipping': 'toggleBilling',
    'click .save-shipping': 'toggleSaveShipping',
    'click .button.submit:not(.loading)': 'submit',
    // 'click .button.paypal:not(.loading)': 'checkoutPaypal',

    'change select.country': 'onCountryChange',
    'change select.state.shipping': 'onShippingStateChange',

    'change form.shipping-address input': 'storeShipping',
    'change form.shipping-address select': 'storeShipping',
    'change form.billing-address input': 'storeBilling',
    'change form.billing-address select': 'storeBilling',


    'click .pay-with-card': 'setPaymentToCard',
    'click .pay-with-paypal': 'setPaymentToPaypal',
  },

  bindings: {
    'model.event_name': '[data-hook=event-name]',
    'model.is_empty': {
      type: 'booleanClass',
      name: 'empty',
    },

    'model.address_required': [
      {
        type: 'booleanClass',
        name: 'address-required',
      },
      {
        type: function (el, val) {
          this.separateBilling = !val;
        }
      },
      {
        type: 'toggle',
        selector: '.row.billing-as-shipping'
      }
    ],
    'model.digitalOnly': {
      type: function (el, val) {
        //this.el.classList.toggle('digital-only', val);
      },
      // type: 'booleanClass',
      // name: 'digital-only',
    },

    'model.paymentMethod': {
      type: function (el, val) {
        el.classList.toggle('payment-card-selected', val === 'card');
        el.classList.toggle('payment-paypal-selected', val === 'paypal');
      },
    },

    'model.mtime': {
      type: function () {
        this.checkForEmptiness();
      },
    },

    // isTotalEmpty: {
    'model.total': {
      type: 'toggle',
      selector: '.payment-details',
    },

    saveShipping: {
      type: 'booleanClass',
      selector: '.row.save-shipping .checkbox',
      name: 'checked'
    },

    separateBilling: [{
      // type: 'booleanClass',
      selector: '.row.billing-as-shipping .checkbox',
      type: function (el, val) {
        el.classList.toggle('checked', !val);
      },
      // invert: true,
      // name: 'checked'
    }, {
      type: 'toggle',
      selector: '.billing-address'
    }],
    shippingCountrySelected: {
      type: 'booleanClass',
      name: 'shipping-country-selected',
    },
    billingCountrySelected: {
      type: 'booleanClass',
      name: 'billing-country-selected',
    },
    requiresDisplayed: {
      type: 'booleanClass',
      name: 'requires-displayed',
    },
    summaryPosition: {
      type: function (el, val) {
        this.el.classList.toggle('summary-static', val === 'static');
        this.el.classList.toggle('summary-fixed-top', val === 'top');
        this.el.classList.toggle('summary-fixed-bottom', val === 'bottom');
      },
    }
  },

  subviews: assign({}, Page.prototype.subviews),

  initialize: function (ops) {
    Page.prototype.initialize.apply(this, arguments);
    ops = ops || {};

    this.eventId = this.model.event.id;

    if (!app.countries) app.countries = new Countries();
    if (!app.states) app.states = new States();

    if (!app.state.bootstrapped) {
      this.listenTo(app.state, 'change:bootstrapped', function () {
        app.state.bootstrapped = true;

        if (this.model.event.complete) {
          this.onReady();
        }
      });
    }
    if (!this.model.event.complete) {
      this.model.event.fetch();
      this.listenTo(this.model.event, 'change:complete', function () {
        if (app.state.bootstrapped) {
          this.onReady();
        }
      });
    }
    if (app.state.bootstrapped && this.model.event.complete) {
      this.onReady();
    }

    this.listenTo(app.state, 'resize', this.resize);
    
    this.saveShipping = !!app.storage.saveShipping;

    var cachedShipping = app.storage.shippingAddress;
    if (cachedShipping) {
      this.cachedShipping = JSON.parse(cachedShipping);
    }

    var cachedBilling = app.storage.billingAddress;
    if (cachedBilling) {
      this.cachedBilling = JSON.parse(cachedBilling);
    }

  },

  render: function () {
    Page.prototype.render.apply(this, arguments);

    if (!this.el) this.renderWithTemplate();
    if (!app.state.bootstrapped) return;

    this.cacheElements({
      leftColEl: '.left-column',
      rightColEl: '.right-column',
      contentsEl: '.checkout-contents',

      expMonthEl: '[name="exp_m"]',
      expYearEl: '[name="exp_y"]',
      cardNumEl: 'input[name="card_num"]',
      cardCodeEl: 'input[name="card_code"]',

      instructionsEl: '.instructions-block textarea',
    });

    if (app.countries.length) {
      this.updateCountries();
    }
    else {
      this.listenTo(app.countries, 'sync', this.updateCountries);
    }

    this.fillAddress('shipping');
    this.fillAddress('billing');

    this.summary = new Summary({
      parent: this,
      model: this.model,
    });
    this.renderSubview(this.summary, this.rightColEl);

    // this.resize();

    // Update shipping type according to photographer's country and buyer's country
    this.updateShippingType(this.model.shippingAddress.country_id);

    // When Pick at option is selected, trigger recalculating tax
    this.listenTo(this.model, 'change:address_required', function (obj, val) {
      this.storeShipping();
    });

    // If the option to pay with a credit card doesn't exist, set the payment method to PayPal
    if (!app.config.checkout_card && app.config.checkout_paypal) {
      this.setPaymentToPaypal();
    }
  },

  getFormData: function (form) {
    var i, len;
    var elements = form.elements;
    var name;
    var data = {};
    for (i = 0, len = elements.length; i < len; i++) {
      name = elements[i].name;
      data[elements[i].name] = elements[i].value;
    }
    return data;
  },

  setPaymentToCard: function () {
    this.model.paymentMethod = 'card';
  },
  setPaymentToPaypal: function () {
    this.model.paymentMethod = 'paypal';
  },

  storeShipping: function (e) {
    if (e && e.target && e.target.name === 'email') {
      this.model.discountCode.setEmail(e.target.value);
    }

    var shipping = this.getFormData(this.query('form.shipping-address'));
    if (!this.model.address_required) {
      shipping.address1 = app.config.photographer.address;
      shipping.address = '';
      shipping.city = app.config.photographer.city;
      shipping.country_id = app.config.photographer.country_id;
      shipping.state_id = app.config.photographer.state_id;
      shipping.zip = app.config.photographer.zip;
    }

    this.model.shippingAddress.set(shipping);
  },
  storeBilling: function () {
    this.model.billingAddress.set(this.getFormData(
        this.query('form.billing-address')));
  },

  fillAddress: function (type) {
    var form = this.query('form.' + type + '-address');
    var model = this.model[type + 'Address'];
    if (!model) return;
    var data = model.toJSON();
    for (var i in data) {
      if (data[i] && i !== 'country_id' && i !== 'state_id') {
        if (form.elements[i]) form.elements[i].value = data[i];
      }
    }
    if (data.country_id && app.countries.length) {
      form.elements.country_id.value = data.country_id;
      this.onCountryChange({target: form.elements.country_id});
    }
  },


  toggleBilling: function () {
    this.separateBilling = !this.separateBilling;
  },

  toggleSaveShipping: function () {
    this.saveShipping = !this.saveShipping;
    app.storage.saveShipping = this.saveShipping ? 1 : '';
    if (this.saveShipping) {
      this.model.shippingAddress.store();
    }
  },

  onCountryChange: function (e) {
    var select = e.target;
    var type = select.classList.contains('shipping') ? 'shipping' : 'billing';
    this[type + 'CountrySelected'] = !!select.value;
    var id = +select.value;
    if (!id) return;
    var country = app.countries.get(id);
    var statesSelect = this.query('select.state.' + type);
    statesSelect.parentNode.classList.add('loading');
    app.states.getStates(country.code, function (states) {
      if (+select.value === id) this.updateStates(statesSelect, states, type);
    }, this);
  },

  onShippingStateChange: function (e) {
    var shipping = this.readFormData('shipping');

    if (shipping && shipping.shipping_country_id) {
      this.updateShippingType(shipping.shipping_country_id);
    }
  },

  // Compare photographer's country and buyer's country,
  // if dfferent, set shipping method to 'International'
  updateShippingType: function (shippingCountryId) {
    if (shippingCountryId !== 0) {
      var photographerCountryId = app.config.photographer.country_id;

      if (photographerCountryId !== shippingCountryId) {
        var intlShipping = app.shipping.find(function (shipping) {
          return shipping.international;
        });

        if (intlShipping) {
          console.log('set international shipping');
          this.model.set('shipping_type', intlShipping.id);
        }
      }
    }
  },

  updateCountries: function () {
    var selects = [this.query('select.country.shipping'),
      this.query('select.country.billing')];
    var select;
    var i, n;
    var country;
    for (i = 0; i < selects.length; i++) {
      select = selects[i];
      select.options.length = 0; // clear out existing items
      select.options.add(new Option('', ''));
      for (n = 0; n < app.countries.length; n++) {
        country = app.countries.models[n];
        select.options.add(new Option(country.name, country.id));
      }

      if (i === 0 && this.cachedShipping.country_id) {
        selects[0].value = this.cachedShipping.country_id;
      }
      else if (i === 1 && this.cachedBilling.country_id) {
        selects[1].value = this.cachedBilling.country_id;
      }
      this.onCountryChange({target: select});
    }
  },
  updateStates: function (select, states, type) {
    var state;
    var cache = type === 'shipping' ? this.cachedShipping : this.cachedBilling;

    select.options.length = 0; // clear out existing items
    select.options.add(new Option('', ''));
    for (var n = 0; n < states.length; n++) {
      state = states.models[n];
      if (+cache.state_id === +state.id) {
        select.options.add(new Option(state.name, state.id, true, true));
      }
      else {
        select.options.add(new Option(state.name, state.id));
      }
    }
    if (states.length === 1) {
      select.value = states.models[0].id;
    }

    // select.value = "47";
    select.parentNode.classList.remove('loading');
  },

  // onShow: function () {
  //   var autofocus = this.query('.autofocus');
  //   if (autofocus) this.after(200, function () {
  //     autofocus.focus();
  //   });
  // },

  readFormData: function (formType) {
    var data = {};
    var el, key;
    var elements = this.queryAll('form.' + formType + '-address input, ' + 'form.' + formType + '-address select');
    var addressFields = ['address1', 'address2', 'city', 'country_id', 'state_id', 'zip'];
    var optionalFields = ['address2'];
    var numericFields = ['country_id', 'state_id'];

    for (var i = 0; i < elements.length; i++) {
      el = elements[i];
      if (el.name === 'email' || el.name === 'phone') {
        key = el.name;
      } else {
        key = formType + '_' + el.name;
      }

      if (!this.model.address_required && formType === 'shipping' && addressFields.indexOf(el.name) > -1) {
        if (el.name === 'address1') {
          data[key] = app.config.photographer.address;
        }
        else if (el.name === 'address2') {
          data[key] = '';
        }
        else {
          data[key] = app.config.photographer[el.name];
        }
      } else {
        if (!el.value && optionalFields.indexOf(el.name) === -1) {
          this.highlightInvalidEl(el);
          return false;
        }

        data[key] = numericFields.indexOf(el.name) > -1 ? +el.value : el.value;
      }
    }

    return data;
  },
  copyBillingData: function (shipping) {
    var data = {};
    data.billing_first_name = shipping.shipping_first_name;
    data.billing_last_name = shipping.shipping_last_name;
    data.billing_address1 = shipping.shipping_address1;
    data.billing_address2 = shipping.shipping_address2;
    data.billing_city = shipping.shipping_city;
    data.billing_country_id = shipping.shipping_country_id;
    data.billing_state_id = shipping.shipping_state_id;
    data.billing_zip = shipping.shipping_zip;
    return data;
  },

  verifyTotals: function () { // for debugging purposes
    var self = this;
    var data = {};

    data.event_id = this.model.event.id;
    data.shipping_type = this.model.shipping_type;
    //if (this.model.digitalOnly) data.shipping_type = 0;
    data.amount = this.model.total;
    // data.amount = 39539;

    data.discount_code = this.model.discountCode.discount_code;
    data.order_note = '';

    xhr({
      uri: app.apiUrl + 'cart/totals/' + this.model.id + '?url=' + app.url +
      '&discount_code='+ data.discount_code +
      '&event_id=' + this.model.event.id +
      '&shipping_state_id=' +
      encodeURIComponent(this.model.shippingAddress.state_id) +
      '&shipping_city=' +
      encodeURIComponent(this.model.shippingAddress.city) +
      '&shipping_zipcode=' +
      encodeURIComponent(this.model.shippingAddress.zip) +
      '&shipping_type=' + encodeURIComponent(this.model.shipping_type),
    }, function (err, resp, body) {
      body = JSON.parse(body);
      if (body.status === 200) {
        self.sent = true;
        console.log('server-side totals', body.data);
      }
      else {
        var error = (body.error && body.error.message) || body.message;
        alert('Totals verification failed' +
            (error ? '\n\nError Message: ' + error : ''));
      }
      self.xhr = null;
    });
    return;
  },

  submit: function (e) {
    if (e && e.altKey) return this.verifyTotals();

    if (this.model.discountCode.loading) return;
    if (this.xhr) return;

    var self = this;
    var billing;
    var shipping;
    //if (!this.model.digitalOnly) {
      shipping = this.readFormData('shipping');
      billing = this.model.total && this.separateBilling ? this.readFormData('billing')
          : this.copyBillingData(shipping);
      if (!shipping) {
        this.alert('Please fill in a complete shipping address.');
        return;
      }
    //}
    //else {
    //  shipping = {
    //    shipping_first_name: '',
    //    shipping_last_name: '',
    //    shipping_address1: '',
    //    shipping_address2: '',
    //    shipping_city: '',
    //    shipping_state_id: 0,
    //    shipping_zip: '',
    //    phone: '',
    //    email: '',
    //  };
    //  billing = this.readFormData('billing');
    //}


    var error = this.validate();
    if (error) return;

    if (this.model.total) {

      if (this.model.paymentMethod === 'card' && !billing) {
        this.alert('Please fill in a complete billing address.');
        return;
      }

      if (this.model.paymentMethod === 'paypal') {
        return this.checkoutPaypal(shipping);
      }
    }

    var payment = {
      card_num: this.query('input[name="card_num"]').value
          .replace(/[^\d]/g, ''),
      card_code: this.query('input[name="card_code"]').value
          .replace(/[^\d]/g, ''),
      exp_date: this.expMonthEl.value + this.expYearEl.value,
    };

    var data = assign({}, shipping, billing, payment);

    data.event_id = this.model.event.id;
    data.shipping_type = this.model.shipping_type;
    data.amount = this.model.total;
    // data.amount = 39539;

    data.discount_code = this.model.discountCode.discount_code;
    data.order_note = this.instructionsEl.value;

    this.query('.button.submit').classList.add('loading');
    
    var keychain = app.keychain.events.get(this.model.event.id);
    var token = keychain ? keychain.token : '';

    this.xhr = xhr({
      uri: app.apiUrl + 'checkout/' + this.model.id +
      '?url='+app.url + '&tok='+ token,
      // uri: app.apiUrl +'checkout/'+ this.model.id +'?url='+ app.url,
      method: 'POST',
      // headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
    }, function (err, resp, body) {
      self.query('.button.submit').classList.remove('loading');
      body = JSON.parse(body);
      if (!err && body && body.status === 200) {
        self.sent = true;

        body.data.order_id = body.data.order_details.order_id;
        body.data.event_id = self.model.event.id;
        var order = new OrderModel(body.data);
        order.store();

        self.model.clear();
        self.model.unset('id');

        app.navigate('events/' + self.model.event.id +
            '/confirmation/' + order.order_id);
      }
      else {
        var error = (body.data && (body.data.description||body.data.error)) ||
            (body.error && body.error.message) || body.message;

        if (Array.isArray(error)) 
          error = error.join('<br><br>');

        self.alert('Error submitting the order.' +
            ((error) ? '<br><br>' + error : ''), {width: 500});
      }

      // if (self.saveShipping) {
        self.model.shippingAddress.store();
      // } else {
      //   self.model.shippingAddress.unstore();
      // }

      self.xhr = null;
    });


  },

  /**
   * Checkout with paypal
   */
  checkoutPaypal: function (shipping) {
    var self = this;
    var cid = this.model.id; // Cart ID
    // var shipping = this.readFormData('shipping');

    // Show loading sign
    this.query('.button.submit').classList.add('loading');

    var note = this.instructionsEl.value;
    app.storage['orderNote' + cid] = note;
    app.storage['orderTotal' + cid] = self.model.total;
    
    var keychain = app.keychain.events.get(this.model.event.id);
    var token = keychain ? keychain.token : '';

    // Start transaction
    self.startTransaction(cid, {
      event_id: self.model.event.id,
      amount: self.model.total,
      tok: token,
      discount_code: self.model.discountCode.discount_code,
      shipping_type: self.model.shipping_type,
      shipping_city: shipping.shipping_city,
      shipping_state_id: shipping.shipping_state_id,
      shipping_zip: shipping.shipping_zip,
      email: shipping.email,
      order_note: note,
    }).then(function (response) {

      var body = response.data;

      // Hide loading sign
      self.query('.button.submit').classList.remove('loading');

      if (response.data.status !== 200) {
        var error = (body.data && (body.data.error || body.data.description)) ||
            (body.error && body.error.message) || body.message;
        if (Array.isArray(error)) error = error.join('<br><br>');

        self.alert('Error submitting the order.' +
            ((error) ? '<br><br>' + error : ''), {width: 500});
        return;
      }

      // if (self.saveShipping) {
        self.model.shippingAddress.store();
      // } else {
      //   self.model.shippingAddress.unstore();
      // }

      // Redirect to paypal
      window.location = response.data.data.redirect_url;
    });


  },

  validate: function () {
    var self = this;
    var cartError = CartPage.prototype.validate.apply(this, arguments);
    if (cartError) {
      app.navigate('events/' + this.model.event.id + '/cart');
      this.alert(cartError);
      return 'cart error';
    }

    if (this.model.total && this.model.paymentMethod === 'card') {
      var payment = {
        card_num: this.cardNumEl.value.replace(/[^\d]/g, ''),
        card_code: this.cardCodeEl.value.replace(/[^\d]/g, ''),
      };
      if (!payment.card_num) {
        this.alert('Please fill in your credit card number.', {
          onOk: function () {
            self.highlightInvalidEl(self.cardNumEl);
          },
        });
        return 'checkout-error';
      }
      if (!payment.card_code) {
        this.alert(
            "Please fill in your credit card's security code. Usually, " +
            "it is the last three digits on the back of the card."
        , {
          onOk: function () {
            self.highlightInvalidEl(self.cardCodeEl);
          },
        });
        return 'checkout-error';
      }
      if (!this.expMonthEl.value) {
        this.alert("Please fill in your credit card's expiration date.", {
          onOk: function () {
            self.highlightInvalidEl(self.expMonthEl);
          },
        });
        return 'checkout-error';
      }
      if (!this.expYearEl.value) {
        this.alert("Please fill in your credit card's expiration date.", {
          onOk: function () {
            self.highlightInvalidEl(self.expYearEl);
          },
        });
        return 'checkout-error';
      }
    }

    return false;
  },

  highlightInvalidEl: function (el) {
    if (el && el.focus) el.focus();
    this.requiresDisplayed = true;
    return 'checkout-error';
  },

});



