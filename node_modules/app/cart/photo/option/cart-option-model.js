...
var CropModel = require('./crop-model');
...
module.exports = BaseModel.extend({
  ...
  derived: {
    ...
    addons_total: {
      ...
      fn: function () {
        ...
        for (var i = 0; i < this.addons.length; i++) {
          total += this.addons.at(i).price * qty;
        }
        return total;
      },
    },
    discounted_price: {
      ...
      fn: function () {
        ...
        if (!option) return 0; // product option
    ...
    shipping_price: {
      ...
      cache: false,
      fn: function () {
        if (!this.photo_option) return 0; // product option
        var shipping_item = this.size.shipping.get(this.cart.shipping_type);
        return shipping_item ? shipping_item.price : 0;
      },
    },
    ...
    bulkPricingAvailable: {
      deps: ['photo_option'],
      fn: function () {
        if (!this.photo_option) return false; // product option
  ...
  initialize: function () {
    this.listenTo(this.addons, 'all', this.updatePrice);
    this.listenTo(this, 'change:qty', this.updatePrice);
  },
  
  updatePrice: function () {
    if (this.collection.isFromProductPhoto) return 0;
    var option = this.photo_option;
    if (!option) { // this is a product option
      return 0;
    }
    var qty = this.size.no_qty ? 1 : this.qty;
    ...
    this.prices = prices;
    // var singlePrice = option.price;
    var addonsTotal = this.addons_total;
    ...
    this.price = prices.total + addonsTotal;
