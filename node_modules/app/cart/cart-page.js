...
var ContactPopup = require('app/misc-popup/cart-email-popup');
...
module.exports = Page.extend({
  viewType: 'cart-page',
  dictionary: 'cart', // localization dictionary 
  pageTitle: 'Cart',
  pageType: 'event-cart',
  ...
  events: {
    ...
    'click .button.open-send-email': 'showCartEmail'
  },
  ...
  checkout: function () {
    ...
    if (error) {
      this.alert(error, { width: 500 });
      return;
    }
    app.navigate('/events/'+ this.model.event.id +'/checkout');
  },

  showCartEmail: function (e) {
  ...
  validate: function () {
    ...
    this.model.updateDigitalOnly();
    if (this.model.digitalOnly && app.config.ignore_min_on_digital_orders) {
      minTotal = 0;
    }

    //The minimum total applies to the subtotal after standard discounts have
    // been applied but before discount_code discounts have been applied.
    var newSubtotal = this.model.subtotal-this.model.discount;
    if (minTotal && newSubtotal+this.model.discount_code_discount < minTotal) {
      return {
        type: 'total too small',
        localizedMessage: this.lz('$total-too-small', 
      ...
      for (var size_id in product.spotsLeft) {
        if (product.spotsLeft[size_id] > 0) {
          return {
            ...
            localizedMessage: this.lz('$product-incomplete', product.name),
          };
        }
        if (product.spotsLeft[size_id] < 0) {
          ...
          return {
            ...
            localizedMessage: this.lz('$product-overfilled',
