  ...
  setData: function (data) {
    ...
    if (Array.isArray(data.general)) {
      for (i = 0; i < data.general.length; i++) {
        // timestamp = Date.parse(data.general[i].expires);
        timestamp = moment(data.general[i].expires);
    ...
    if (Array.isArray(data.size_override)) {
      for (i = 0; i < data.size_override.length; i++) {
        ...
        // timestamp = Date.parse(discount.expires);
        timestamp = moment(discount.expires);
        if (!timestamp || timestamp <= now) continue;
