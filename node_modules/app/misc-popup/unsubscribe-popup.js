    ...
    this.listenTo(this.model, 'change:name', function () {
      this.getEmailSubscriptions(this.eid, this.prefId, this.email).then(function (response) {
        ...
        if (response.data.status === 200) {
          ...
          self.alert('Failed to get subscription settings. ' +
              'Please check your internet connection and try again.');
        }
      }, function (error) {
        self.alert('There was an error in getting subscription settings. ' + error);
      });
    });
  ...
  save: function () {
    ...
    this.updateEmailSubscriptions(this.eid, this.prefId, this.email, this.data).then(function (response) {
      ...
      if (response.data.status === 200) {
        ...
        self.alert('Failed to save subscription settings. ' +
                'Please check your internet connection and try again.');
      }
      self.saving = false;
    }, function (error) {
      self.alert('There was an error in updating subscription settings. ' + error);
