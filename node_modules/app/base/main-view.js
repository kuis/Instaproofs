...
var EmailRequestPopup = require('app/misc-popup/email-request-popup');
var MainenanceView = require('app/event/maintenance/event-maintenance');
var OfflineView = require('app/offline/offline-overlay');
...
module.exports = BaseView.extend({
  ...
  initialize: function () {
    ...
    this.listenTo(this.model, 'change:currentEventId', function () {
      console.log('event id', this.model.currentEventId);
      this.updateMaintenanceStatus();
      if (this.model.currentEventId) this.updateCartStatus();
    });

    this.listenTo(this.model, 'change:offline', function () {
      console.log('update offline', this.model.offline);
    ...
    // show email input when email is required (e.g. when saving favs)
    this.listenTo(app, 'email-required', this.requireEmail);
  },
  ...
  bindings: {
    ...
    'model.enable_cart': {
      type: function (el, val) { el.classList.toggle('no-cart-store', !val); }
    },
  ...
  // display a popup with an input field
  requireEmail: function (event, success) {
    this.popup(new EmailRequestPopup({
      success: function (email, callback) {
        app.keychain.saveEventKeys(event, email, '', true);
        if (typeof success === 'function') success(email, callback);
      },
    }));
  },

  updateCartStatus: function () {
    var eid = this.model.currentEventId;
    var event = eid && app.eventModels[eid];
    var enableCart = event ? event.enable_cart : true;
    this.model.enable_cart = enableCart;
  },

  updateMaintenanceStatus: function () {
    var eid = this.model.currentEventId;
    var event = eid && app.eventModels[eid];
    var maintenance = !!event && event.server_active === false;
    // maintenance = !!event;
    
    // remove old maintenance view if needed
    if (this.maintenanceView && (!maintenance || 
        this.maintenanceView.model.id !== eid)) {
      this.maintenanceView.remove();
      this.maintenanceView = null;
    }

    if (maintenance && !this.maintenanceView) { // display maintenance overlay
      this.showMaintenanceView(event);
    }
  },
  showMaintenanceView: function (event) {
    this.maintenanceView = new MainenanceView({
      model: event,
    });
    document.body.appendChild(this.maintenanceView.el);
  },


  updateOfflineStatus: function () {
    // remove old overlay view if needed
    if (!this.model.offline && this.offlineView) {
      this.offlineView.remove();
      this.offlineView = null;
    }

    if (this.model.offline && !this.offlineView) { // display offline overlay
      this.showOfflineView();
    }
  },
  showOfflineView: function (event) {
