...
module.exports = require('ampersand-view').extend({
  ...
  lz: function () {
    ...
    var result = app.i18n.localize(this.lzData[lzId]);
    
    return '<span data-lz-id="'+ lzId +'">'+ result +'</span>';
  },
  
  // similar to lz(), but for attributes like `placeholder`
  // first argument is attribute's name, others are the same as lz()
  // returns an object containing attributes to be assigned to the element 
  lzA: function () {
    ...
    var localizedString = app.i18n.localize(this.lzData[lzId]);
    
    var result = {
      'data-lz-id': lzId,
      'data-lz-attribute': attrName,
    };
    result[attrName] = this.escape(localizedString);
  ...
  updateLocalizedContent: function () { // updates all localizable text 
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      attr = elements[i].getAttribute('data-lz-attribute');
      if (!this.lzData[id]) continue; // element is from a subview 
      str = app.i18n.localize(this.lzData[id]);
      if (attr) {
        elements[i].setAttribute(attr, str);
      }
      else {
  ...
  alert: function (notification, options) {
    options = typeof notification === 'object' ? notification : (options || {});
    if (typeof notification !== 'object') options.notification = notification;
    
    if (options.localizedMessage) {
      options.content = options.localizedMessage;
    }
    else if (options.notification) {
      options.content = this.lz(options.notification);
    }
    else if (options.message) {
      options.content = this.lz(options.message);
    }
    
    options.context = this;
    return modalPopup.alert(options);
  },
  confirm: function (notification, options) {
    options = typeof notification === 'object' ? notification : (options || {});
    if (typeof notification !== 'object') options.notification = notification;
  ...
  notify: function (content, options) {
    ...
    return NotificationPopup.show(options);
  },
...
var NotificationPopup = require('app/notification-popup/notification-popup');
