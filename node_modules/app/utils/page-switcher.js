...
module.exports = ViewSwitcher.extend({
  ...
  switcher: function (done, newView, oldView) {
    ...
    $newEl.removeClass('hidden no-animation active inactive');
    
    schedule(function () {
      app.state.hiddenNav = false;
      if (newView.fixedFooter) {
        $(newView.fixedFooter.el).removeClass('hidden');
        this.$footer.append(newView.fixedFooter.el).removeClass('hidden');
      }
      else {
    ...
    var onTransitionEnd = function () {
      ...
      schedule(function () {
        ...
        if (newView === self.current) $newEl.addClass('active');
        
        // avoid a webkit bug with disappearing scrollbar
        // $newEl[0].style[ua.css.transform] = 'translateY(0)';
      }, 'high');
      
      if (newView === this.current) done();
    ...
    schedule(function () {
      if (oldView) {
        ...
        $(oldView.el).removeClass('no-animation active').addClass('hidden');
        if (oldView.fixedFooter) {
          $(oldView.fixedFooter.el).addClass('hidden');
        }
        ...
        if (app.state.noTransitions) {
          ...
          $newEl.one(ua.events.animationEnd, onTransitionEnd);
          setTimeout(onTransitionEnd, 1000); // fallback just in case
        }
      }
      else {
        $newEl.addClass('no-animation active');
