...
module.exports = BaseView.extend({
  ...
  events: {
    ...
    'keyup .search-bar .event-search': 'onEventSearchChange',
    'paste .search-bar .event-search': 'onEventSearchChange',
    'change .search-bar .event-search': 'onEventSearchChange',

    'keyup .search-bar .photo-search': 'onPhotoSearchChange',
    'paste .search-bar .photo-search': 'onPhotoSearchChange',
    'change .search-bar .photo-search': 'onPhotoSearchChange',
  },
  ...
  props: {
    ...
    eventSearch: ['boolean', true, false],
    photoSearch: ['boolean', true, false]
  },
  ...
  bindings: {
    searchString: {
      type: function (el, val) {
        ...
        app.state.photosSearchString = val.length >= 3 ? val : '';
      }
    },
    menuDisplayed: {
      type: 'booleanClass',
  ...
  initialize: function () {
    ...
    this.onEventSearchChange = debounce(this.onEventSearchChange, 500);
    this.onPhotoSearchChange = debounce(this.onPhotoSearchChange, 500);
    
    // this.renderWithTemplate();
    this.el.innerHTML = this.template();
    this.cacheElements({
      eventSearchInput: '.search-bar input.event-search',
      photoSearchInput: '.search-bar input.photo-search'
    });
    ...
    if (!app.state.bootstrapped) {
      this.listenToOnce(app.state, 'change:bootstrapped', function () {
        ...
        this.query('.logo').target = app.config.logo_link !== '' && 
            app.config.logo_link.indexOf('instaproofs.com') < 0
              ? '_blank' 
              : '_self';
      });
    }
    
    this.listenTo(app, 'reload-images', function () {
      if (!app.state.bootstrapped) return;
      this.query('.logo img').removeAttribute('src');
  ...
  onEventSearchChange: function (e) {
    if (e && e.keyCode === 27) return this.quitSearch();
    var str = this.eventSearchInput.value;
    this.searchString = str.length >= 3 ? str : '';
  },

  onPhotoSearchChange: function (e) {
    if (e && e.keyCode === 27) return this.quitSearch();
    var str = this.photoSearchInput.value;
    this.searchString = str.length >= 3 ? str : '';
  },
  ...
  toggleSearch: function (e) {
    ...
    if (this.searchDisplayed) {
      if (!this.eventSearchInput.className.includes("hidden")) {
        this.eventSearchInput.value = '';
        this.eventSearchInput.blur();
      } else if (!this.photoSearchInput.className.includes("hidden ")) {
        this.photoSearchInput.value = '';
        this.photoSearchInput.blur();
      }
    }
    this.searchDisplayed = !this.searchDisplayed;
    if (this.searchDisplayed) {
      if (!this.eventSearchInput.className.includes("hidden")) {
        this.eventSearchInput.focus();
      } else if (!this.photoSearchInput.className.includes("hidden")) {
        this.photoSearchInput.focus();
      }
      if (this.menuDisplayed) this.toggleMenu();
      if (app.state.noticesDisplayed) app.view.hideNotices();
    }
