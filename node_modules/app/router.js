...
module.exports = require('ampersand-router').extend({
  ...
  showLightbox: function (eid, cid, pid, back, buy) {
  ...
  initialize: function() {
    ...
    // single photo ==========================================================
    this.route(/^events\/(\d+)\/(\d+)\/(\d+)(buy)?$/, 
        function (eid, cid, pid, buy) {
      ...
      if (!buy || this.currentPage !== 'cart') this.openPage('category', {
        id: eid+'_'+cid,
        cid: cid,
        model: event,
        showMore: false,
      ...
      if (!app.isReady) {
        ...
        this.showLightbox(eid, cid, pid, 
            buy && this.currentPage === 'cart' && ('#events/'+eid+'/cart'),
            !!buy);
      }
