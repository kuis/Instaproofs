...
module.exports = Router.extend({
  ...
  initialize: function() {
    ...
    // homepage ==============================================================
    this.route(/^(events(?:\?(.*?))?)$/, function (fullUrl, query) {
    ...
    this.route(/^(paypal_confirmation(?:\?(.*?))?)$/, function (fullUrl, query) {
      this.currentParameters = {};
      if (lightbox) this.hideLightbox();
      ...
      if (!query['token']) {
        alert('Paypal transaction token is missing');
        return app.navigate('events');
      }

      if (!app.storage['transaction']) {
        alert('Paypal transaction info is missing');
        return app.navigate('events');
      }

      var token = query['token'];
      var transaction = JSON.parse(app.storage['transaction']);

      if (token !== transaction['token']) {
        alert('Paypal transaction info is invalid');
        return app.navigate('events');
      }

      var eid = transaction.eid;
      var event = app.eventModels[eid]
          || (pages.home && pages.home.list.collection.get(eid))
          || new EventModel({ id: eid });
      app.eventModels[eid] = event;
    ...
    // paypal cancel page =========================================================
    this.route(/^(paypal_cancel(?:\?(.*?))?)$/, function (fullUrl, query) {
      this.currentParameters = {};
      if (lightbox) this.hideLightbox();
      ...
      var token = query['token'];
      if (app.storage['transaction']) {
        app.storage.removeItem('transaction');
      }

      alert('You cancelled the paypal payment');
