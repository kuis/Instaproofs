...
var PhotoView = require('app/views/category/thumb');
...
module.exports = Parent.extend({
  ...
  initialize: function (ops) {
    ...
    if (this.collection.length) {
      this.resize();
      if (this.collection.length) {
        this.scheduleWrite(this.updateViewsList, 'normal');
      }
    }
    else {
  ...
  setHeight: function () {
    ...
    if (this.hiddenPresent && this.hiddenPositioned) {
      if (this.hiddenDisplayed) {
        height = _.max(this.columns, returnHeight).height;
      }
      else {
  ...
  showPhotos: function (initial) {
    ...
    for (i = 0; i < this.displayedCount; i++) {
      ...
      if (!app.state.noTransitions && initial && 
          _.min(this.columns, returnHeight).height > app.state.height - 200) {
        break;
      }
    }
    if (initial) this.displayedCount = i;
    this.scheduleWrite(this.setHeight, 'high');
  },
  ...
  showPhoto: function (photo, initial) {
    ...
    if (model.hidden && !this.hiddenPositioned) {
      ...
      this.mainHeight = _.max(this.columns, returnHeight).height;
