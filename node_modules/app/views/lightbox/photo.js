...
module.exports = Parent.extend(bindTransforms).extend({
  ...
  bindings: {
    inCart: {
      type: function (el, val) {
        ...
        this.query('.action.cart .tip').innerHTML = val ? 'In Cart' : 'Buy Image';
      },
      // type: 'booleanClass',
      // name: 'in-cart',
    },
  ...
  initialize: function () {
    ...
    this.img.addEventListener(ua.events.transitionEnd, this.onImageTransitionEnd);
    this.resize();
  },
  ...
  resize: function (ops) {
    ...
    if (this.parent.buyFormDisplayed) {
      ...
      pHeight = app.state.fullscreen || ua.mobile ? app.state.height : app.state.height - 60;
    }
    ...
    if (this.buyForm && this.parent.buyFormDisplayed) {
      var formWidth = app.state.width - this.imageWidth - 90;
      var formLeft = 30;
      this.imagesRight = this.parent.buyFormDisplayed ? 30 :
                         app.state.fullscreen || ua.mobile ? 0 : 54;
      if (formWidth > 650) {
    ...
    if (!ops.notReally) {
      ...
      // if (this.buyForm) {
      //   this.buyForm.el.style.width = app.state.width - this.imageWidth - 90 + 'px';
      // }
    }
  ...
  hideBuyForm: function () {
    ...
    var newTop = (app.state.height - this.imageHeight - 
                 (this.model.name ? 54 : 0)) / 2;
    // var newTop = (app.state.height - this.imageHeight) / 2;
