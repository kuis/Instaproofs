...
module.exports = {
  ...
  slideshowSpeed: ua.mobile ? 4000 : 5000, // amount of time in ms between slide changes 
  ...
  toggleSlideshow: function () {
    if (this.slideshow) { // stop the show 
      ...
      if (this.music) this.audio.pause();
    }
    else { // start the show
      this.slideshow = true;
      this.slideshowTimer = setTimeout(this.slideshowCallback, this.slideshowSpeed);
  ...
  speedDown: function (e) { // slideshow speed dial mouse/touch-down
    var x0 = this.speedSliderX = this.$('.slideshow-speed').offset().left;
  ...
  onMouseRest: function () { // no mouse or touch activity for a while
    ...
    // if (!$target.closest('.slideshow-controls, .top-right, .actions').length) {
    this.mouseOverActions = !!$target.closest('.actions').length;
    this.mouseOverTopRight = !!$target.closest('.top-right').length;
    this.mouseOverSlideshow = !!$target.closest('.slideshow-controls').length;
    // if (!$target.closest('.top-right, .actions').length && !this.speedSliderX) {
  ...
  toggleMusic: function () {
    if (!this.music) {
      // this.audio.load();
      this.audio.play();
      // this.musicToggle.setAttribute('title', 'Stop Music');
    }
    else {
      this.audio.pause();
      // this.musicToggle.setAttribute('title', 'Play Music');
    }
