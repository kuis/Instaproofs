...
module.exports = require('ampersand-state').extend({
  ...
  initialize: function (options) {
    ...
    // var photoWidth = this.photo.model.original_width;
    
    this.ratio = this.option.model.width / this.option.model.height;
    
    // make sure print size is the same orientation as the photo 
    if (this.ratio > 1 && photoRatio < 1 || this.ratio < 1 && photoRatio > 1) {
      this.inchesWidth = this.option.model.height;
      this.inchesHeight = this.option.model.width;
    }
    else {
      this.inchesWidth = this.option.model.width;
      this.inchesHeight = this.option.model.height;
    }
    ...
    // this.canRotate = minDpi * this.inchesHeight / photoWidth * 100 < 90;
        // this.photo.model.original_width >= minDpi * this.inchesHeight;
  ...
  reset: function (options) {
    var photoWidth = this.photo.model.original_width;
    var photoHeight = this.photo.model.original_height;
  ...
  rotate: function () {
    ...
    // stick stick to fixed min size of 1/3 for now
  ...
  getLargestOverflow: function (crop) {
    ...
    for (var i in crop) {
      ...
      amount = i === 'left' || i === 'right' 
          ? -crop[i] * model.original_width 
          : -crop[i] * model.original_height;
  ...
  scale: function (f, xSide, ySide) {
    ...
    var height = (width * model.original_width / this.ratio) /
                 model.original_height;
