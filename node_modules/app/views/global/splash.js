...
module.exports = Parent.extend({
  ...
  hide: function (e) {
    if (!app.isReady || this.unlocked || this.model && this.model.locked) {
      return;
    }
  ...
  showLoginForm: function (event) {
    ...
    // console.log('display_image', event.display_image);
    // event.display_image = false;
    ...
    // }
    // else {
    //   this.query('.image').style.backgroundImage = 'none';
    // }
    ...
    this.loginForm = true;
    this.query('.label').innerHTML = app.templates.global.login(this);
    // this.query('.image').style.backgroundImage = 'url('+
    //     event.img_url.replace(/w=\d+/, 'w='+ app.state.width)
    //     .replace(/h=\d+/, 'h='+ (app.state.height - 100)) +')';

    if (this.model.email) this.query('.email input').focus();
