...
var PhotoView = require('app/views/lightbox/photo');


module.exports = Parent.extend(require('app/views/lightbox/slideshow-mixin'),
                               require('app/views/lightbox/touch-mixin'),
                               require('app/views/lightbox/buy-mixin'),
{
  props: {
    ...
    mouseOverSlideshow: ['boolean', true, false], // mouse over slidshow controls
  ...
  initialize: function () {
    ...
    this.slideshowCallback = this.slideshowCallback.bind(this);
    this.onMouseRest = this.onMouseRest.bind(this);
  ...
  reset: function (ops) {
    ...
    this.hasMusic = !!category.music;
    if (category.music) {
      if (category.music !== this.currentTrack) {
        this.audio.setAttribute('src', category.music);
        // this.audioReady = false;
        if (this.music) this.audio.play();
      }
      this.currentTrack = category.music;
    }
    else {
  ...
  forward: function (options) {
    ...
    // if (!this.next.length) this.preloadMore();
  ...
  updateSlides: function () { // keep only 3 slides in the DOM 
    // console.log('prev', this.prev[0] && this.prev[0].model.url);
    // console.log('current', this.current.model.url);
    // console.log('next', this.next[0] && this.next[0].model.url);
    var i;
    for (i = 0; i < this.prev.length; i++) {
    ...
    for (i = 0; i < this.next.length; i++) {
      // console.log('add next slide');
      // if (i === 0) console.log(this.next[i].el.parentNode);
  ...
  preloadMore: function (options) { // preload a photo if needed
    ...
    this.preloading = true;
    // if (which === 'next' && this.next.length >= 2) return this.updateSlides();
