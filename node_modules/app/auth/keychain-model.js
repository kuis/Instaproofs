...
var EventKeysModel = require('app/base/base-model').extend({
  props: {
    ...
    allowedFullDownload: ['boolean', true, false],
...
module.exports = require('app/base/base-model').extend({
  ...
  clear: function (currentId) { // logout from all events except currently open
    ...
    if (!currentEventId) {
      for (i = 0; i < this.events.length; i++) {
        ...
        app.trigger('event-logout', event.id);
      }
      this.events.reset();
  ...
  confirmKeys: function (id, permissions, token) {
    ...
    model.allowedFullDownload = permissions.indexOf('event_download') > -1;
    // if (changed) {
      this.mtime = Date.now();
      this.store();
    // }
