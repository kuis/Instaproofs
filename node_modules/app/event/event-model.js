...
module.exports = BaseModel.extend({
  ...
  props: {
    ...
    // whether this event should have purchasing functionality
    enable_cart: ['boolean', true, true],
    ...
    server_active: ['boolean', true, true], // whether photos are online
  ...
  derived: {
    ...
    location: {
      ...
      fn: function () {
        ...
        if (this.state) a.push(this.state);
        return a.join(', ');
      },
    },
    ...
    alt_homepage: {
      ...
      cache: false,
      fn: function () {
        if (this.name && this.categories && this.categories.length === 1 && 
            this.products && !this.products.length) {
          if (!this.categories.length) {
            console.log('no cat', this.toJSON());
          }
        // if (this.name) {
          return 'events/'+ this.id +'/'+ this.categories.at(0).id;
        }
        else {
          return false;
        }
  ...
  fetch: function (options) {
    ...
    options.success = function (model, res, options) {
      if (res.status !== 200) {
        ...
        app.keychain.confirmKeys(model.id, res.permissions, res.token);
        ...
        if (model.alt_homepage && app.view.currentPage.pageType === 'event' &&
            app.view.currentPage.model.id === model.id) {
          app.router.history.navigate(model.alt_homepage, { 
            trigger: true, replace: true });
        }

        app.view.updateMaintenanceStatus();
        app.view.updateCartStatus();
      }
      if (success) success.call(model, model, res, options);
    };
    options.error = function (model, res, options) {
      app.trigger('event-fetch-error', model.id);
      if (error) error.call(model, model, res, options);

      app.state.setOffline();
      
      if (model.locked) {
        // app.router.login(model);
      } 
      else {
        model.listenTo(app, 'online', model.fetch);
        // model.fetch();
      }
    };
  ...
  initialize: function () {
    ...
    this.checkServerStatus = debounce(this.checkServerStatus, 1000);

    this.listenTo(this, 'change:server_active', this.onServerStatusChange);

    if (this.server_active === false) this.onServerStatusChange();
  },

  onServerStatusChange: function () {
    app.view.updateMaintenanceStatus();
    console.log('change:server_active', this.server_active);
    if (this.server_active !== false) {
      app.state.reloadImages();
      this.stopServerMonitoring();
    }
    else this.startServerMonitoring();
  },


  // check if photos server is online
  checkServerStatus: function (cb) {
    var url = app.apiUrl +'event/active/'+ this.id +'?url='+ app.url;
    var self = this;
    xhr.get(url, function (err, resp, body) {
      ...
      if (!app.state.offline && body && body.data) {
        self.server_active = !!body.data.server_active;
      }
      if (!app.state.offline && self.server_active) {
        app.state.reloadImages();
      }
      if (typeof cb === 'function') cb(self.server_active);
    });
  },

  startServerMonitoring: function () {
    if (this.serverStatusIntervalId) return;
    if (this.server_active) return this.stopServerMonitoring();
    var self = this;
    this.serverStatusIntervalId = setInterval(function () {
      self.checkServerStatus();
    }, 1500);
  },
  stopServerMonitoring: function () {
    clearInterval(this.serverStatusIntervalId);
  ...
  parse: function (res) {
    // app.storage.eventData = JSON.stringify(res);
    var self = this;
    var i, j, option;
    ...
    // data.server_active = false;
    // this.id = data.id;
    // this.checkServerStatus();
    // if (data.id === 1419576) data.enable_cart = false;
    ...
    if (res.cart && res.cart.id) {
      ...
      if (!('shipping_type' in res.cart)) {
        var storedData = app.storage['cart'+ this.id];
        if (storedData) {
          storedData = JSON.parse(storedData);
          res.cart.shipping_type = storedData.shipping_type;
        }
      }
