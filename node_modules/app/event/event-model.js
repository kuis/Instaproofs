...
module.exports = BaseModel.extend({
  ...
  derived: {
    ...
    thumb_url: {
      ...
      fn: function () {
        ...
        var url = helpers.toThumbUrl(this.img_url);
    ...
    //   deps: ['img_url'],
    //   fn: function () {
    //     return helpers.toBlurUrl(helpers.toThumbUrl(this.img_url));
    //   },
    // },
  ...
  parse: function (res) {
    // console.log('event parse', res.status);
    var i;
    ...
    //   // console.log('saving event data', this.id);
    //   // save data into local storage
    //   app.storage['eventStorageTime'+ this.id] = Date.now();
    //   res.savedLocally = true;
    //   app.storage['eventData'+ this.id] = JSON.stringify(res);
    // }
    ...
    // for (i = 0; i < discounts.general.length; i++) {
    //   if (!discounts.general[i].expires) {
    //     discounts.general[i].expires = '2015-12-31';
    //   }
    // }
    // for (i = 0; i < discounts.size_override.length; i++) {
    //   if (!discounts.size_override[i].expires) {
    //     discounts.size_override[i].expires = '2015-12-31';
    //   }
    // }
    // if (!data.discounts.general || !data.discounts.general.length) {
    //   data.discounts.general = [
    //     {
    //       size_discount: 0,
    //       product_discount: 0.15,
    //       expires: '2016-01-25T00:00:00-07:00',
    //       description: 'Products are 15% off',
    //     },
    //     {
    //       size_discount: 0.10,
    //       product_discount: 0,
    //       expires: '2016-01-25T20:00:00-07:00',
    //       description: 'Prints are 10% off',
    //     },
    //   ];
    // }
    // if (!data.discounts.size_override || !data.discounts.size_override.length) {
    //   data.discounts.size_override = [
    //     {
    //       id: 14,
    //       discount: 0.20,
    //       // expires: '2015-11-08T00:00:00-01:00',
    //       expires: Date.now() + 47 * 3600 * 1000,
    //       description: '4x6 are 20% off',
    //     },
    //     {
    //       id: 16,
    //       discount: 0.20,
    ...
    //       // expires: '2015-11-08T00:00:00-01:00',
    //       expires: Date.now() + 47 * 3600 * 1000,
    //       description: 'Wallets are 25% off',
    //     },
    //     {
    //       id: 71,
    //       discount: 0.25,
    ...
    //     size_discount: 0.10,
    //     product_discount: 0,
    //     expires: '2016-02-25 20:00:00T20:00:00-07:00',
    //     // expires: '2016-02-09 00:00:00T00:00:00-10:00',
    //     description: 'Prints are 10% off',
    //   },
    // ];
    ...
    if (res.cart && res.cart.id) {
      for (var i = 0; i < res.cart.photos.length; i++) {
        if (res.cart.photos[i].color_id === 1) {
          res.cart.photos[i].color_id = 7;
        }
      }
    ...
    for (i = 0; i < data.categories.length; i++) {
      ...
      if (cat.photos && cat.photos.length) {
        for (j = 0; j < cat.photos.length; j++) {
          if (cat.photos[j].hidden) {
    ...
    if (fullData) {
      ...
      for (i = 0; i < data.categories.length; i++) {
        ...
        if (cat.photos && cat.photos.length) {
          for (j = 0; j < cat.photos.length; j++) {
            if (cat.photos[j].hidden) {
              hidden.push({ pid: cat.photos[j].id, cat_id: cat.id });
            }
          }
        }
      }
