/*==========================================================================*\
  A single event on the homepage
\*==========================================================================*/
"use strict";

var BaseView = require('app/base/base-view');

var app = require('app');
var config = app.config;
var templates = require('app/templates');

module.exports = BaseView.extend({
  viewType: 'event-item',
  template: templates.event['event-item'],

  bindings: {
    'model.server_active': {
      type: function (el, val) {
        el.classList.toggle('maintenance', !val);
      },
    },
  },

  initialize: function () {
    BaseView.prototype.initialize.apply(this, arguments);
    this.listenTo(app, 'reload-images', this.render);
  },

  render: function () {
    if (config.style === 'list') {
      this.template = templates.event['event-row'];
    }
    this.renderWithTemplate(this);

    this.cacheElements({
      bgEl: '.bg',
      eventTitleEl: '.event-title'
    });

    this.el.classList.toggle('password-protected', 
                             this.model.password && this.model.locked);
    this.el.classList.toggle('showcased', this.model.showcase);

    if (config.style !== 'list') {
      // Set image url
      var imageUrl = this.model.thumb_url || config.no_images_url;
      if (!this.model.display_image) {
        imageUrl = config.private_gallery_url;
      }
      this.bgEl.style.backgroundImage = 'url('+ imageUrl +')';

      // Set image position
      if (this.model.img_position) {
        this.bgEl.style.backgroundPosition = this.model.img_position;
      }

      // Set event title class
      var len = this.model.name.length;
      var titleClass = len >= 25 ? 
                       'long-' + Math.min(10, Math.floor((len - 25) / 10)) : '';
      if (titleClass) {
        this.eventTitleEl.classList.add(titleClass);
      }
    }

    return this;
  }

});
