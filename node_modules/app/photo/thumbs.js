...
module.exports = BaseView.extend({
  ...
  initialize: function (ops) {
    ...
    this.listenTo(app, 'reload-images', this.loadAllDisplayed);
    ...
    this.viewsById = {};
    this.renderQueue = [];
    this.searchString = app.state.photosSearchString;
    ...
    this.listenTo(app.state, 'change:photosSearchString', function () {
      var str = app.state.photosSearchString;
      this.searchString = str;
      console.log('change search', str);

      var nodes = this.queryAll('.grid-photo');
      ...
      this.searchPhotoCount = 0;
      this.updateViewsList();
      this.showMore();

      if (this.searchPhotoCount == 0) {
        this.parent.parent.empty = true;
      } else {
        this.parent.parent.empty = false;
      }
  ...
  getPhotoView: function (model) {
    var id = model.id;
    if (model.name.toLowerCase().includes(this.searchString.toLowerCase())) {
    ...
    for (var i = 0; i < this.collection.length; i++) {
      ...
      if (photo !== undefined) {
        if (photo.model.hidden) hidden.push(photo);
          ...
          this.photoViews.push(photo);
          this.searchPhotoCount++;
        }
      }
    }
  ...
  cleanup: function () {
    ...
    if (this.displayedCount > this.photoViews.length) {
      this.displayedCount = this.photoViews.length;
    }
  },
  ...
  showPhotos: function (initial) {
    ...
    var counts = this.displayedCount >= this.photoViews.length ? this.photoViews.length : this.displayedCount;
    ...
    for (i = 0; i < counts; i++) {
      ...
      if (this.photoViews[i]) this.showPhoto(this.photoViews[i], initial);
  ...
  showPhoto: function (photo, initial) {
    if (!photo) {
      console.log('no thumb to display');
      console.log('this.photoViews', this.photoViews.length);
    ...
    if (model.hidden && !this.hiddenPositioned) {
      ...
      this.mainHeight = maxBy(this.columns, returnHeight).height || 0;
      this.hidesHeader.style[ua.css.transform] = 
          'translateY('+this.mainHeight + 'px)';
      for (i = 0; i < this.columns.length; i++) {
        this.columns[i].height = (this.mainHeight + 120) || 0;
      }
    }
  ...
  showMore: function () {
    ...
    if (remaining <= max) {
      this.write(function () {
        ...
        this.parent.parent.query('#prev-next-container').classList.remove('hidden');
        if (this.parent.parent.fixedFooter1 !== undefined && !this.parent.parent.empty)
          this.parent.parent.fixedFooter1.el.classList.add('hidden');
      });
    }
