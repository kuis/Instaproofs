...
var BuyOption = BaseModel.extend({
  ...
  getPrices: function (qty) {
    ...
    if (this.discount && this.discount.discount) {
      ...
      //prices.discountAmount = this.round5ToFloor(this.discount.discount * prices.bulk);
    ...
    if (bpType === 'compound' && this.bulk_pricing.length) {
      ...
      var singlePrice = Math.round(prices.total / qty);
      prices.total = Math.round(singlePrice * qty);
      prices.discounted = Math.round(singlePrice - 
                                     ((1-discountMP) * singlePrice));
    }
...
module.exports = BaseModel.extend({
  ...
  derived: {
    ...
    cat_id: {
      ...
      fn: function () {
        if (!this.collection) {
          if (this._cat_id) return this._cat_id; // favorites
          return;
        }
  ...
  initialize: function () {
    ...
    //     this.collection.parent.photos && !this.collection.length) {
    //   return;
    // }
    BaseModel.prototype.initialize.apply(this, arguments);
  ...
  getUrl: function (colorId) {
    ...
    if (typeof colorId === 'string') {
      ...
      if (!color) {
        console.warn('color not found', colorId, app.colors.toJSON());
        console.trace();
        color = app.colors.getDefaultColor();
      }
      style = color.prefix;
    }
    ...
    var url = this.img_url.replace(/&w=\d+&h=\d+/g, 
                                   '&w='+ width +'&h='+ height); 
