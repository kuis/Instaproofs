...
module.exports = BaseView.extend({
  ...
  props: {
    ...
    // current selection matches the one saved as default
    isDefaultSelection: ['boolean', true, false],
  },
  ...
  events: {
    ...
    'click .make-default .button': 'clearDefaultSelection',
  ...
  initialize: function () {
    ...
    // if (ops && ops.cropOnly) { 
    if (app.history.latestConfig.crop) { 
      this.cropOnly = true; 
    }
    ...
    if (!this.parent.model.in_cart && app.buyPresets[this.model.event.id]) {
      this.fillFromPreset(app.buyPresets[this.model.event.id]);
    }
  },
  ...
  render: function () {
    ...
    this.optionsView = this.renderCollection(
      ...
      filter: function (model) {
        var downSize = model.size.download_size;
        if (downSize && self.model.width < downSize && 
            self.model.height < downSize) {
          return false;
        }
        return true;
      },
    ...
    this.after(50, this.updatePrice);
  },
  
  fillFromPreset: function (preset) {
    try {
      var i, j, option, addons, addonPreset, sizePreset, product;
      for (i = 0; i < this.optionsView.views.length; i++) {
        option = this.optionsView.views[i];
        sizePreset = preset.main[option.model.size.id+'-'+option.model.color_id];
        if (sizePreset) {
          option.qty = sizePreset.qty;
          option.qtyView.value = sizePreset.qty;
          option.active = true;
          if (option.addonsView) {
            ...
            for (j = 0; j < addons.length; j++) {
              ...
              addonPreset = sizePreset.addons[addons[j].model.id];
              if (!addonPreset) continue;
              
              if (addons[j].model.options.length <= 1) {
                addons[j].active = true;
              }
              else {
                addons[j].selected_id = addonPreset;
              }
      ...
      for (i = 0; i < this.productsView.views.length; i++) {
        ...
        for (j = 0; j < product.options.length; j++) {
          ...
          sizePreset = preset.products[product.model.cart_item_id +'-'+ 
                                       option.model.id +'-'+ option.colorId];
          if (sizePreset) {
            option.qty = sizePreset.qty;
            option.active = true;
            option.updatePrice();
  ...
  prefill: function (cartPhotos) {
    ...
    for (i = 0; i < this.optionsView.views.length; i++) {
      ...
      option.active = true;
      option.qty = item.qty;
      if (option.qtyView) option.qtyView.value = item.qty;
    ...
    if (this.parent.model.in_cart) {
      this.query('.button.save').innerHTML = this.lz('Save');
    }
  },
  
  selectVariant: function (e) {
    var nameEl = e.target.closest('.variant-name');
    var id = nameEl.getAttribute('data-color-id');
  ...
  toggleSaveAsDefault: function () {
    if (this.isDefaultSelection) return;
  ...
  updatePrice: function () {
    ...
    for (var i = 0; i < this.optionsView.views.length; i++) {
      option = this.optionsView.views[i];
      total += option.price;
      if (option.active) isEmpty = false;
    }
    this.totalPrice = total;
    
    isEmpty = isEmpty && !this.query('.product-size.active');
    this.empty = !this.model.in_cart && isEmpty;
    
    this.checkForDefaultSelection();
  },
  
  clearDefaultSelection: function (confirmed) {
    if (confirmed !== true) {
      this.confirm({
        message: "This will clear your saved default selection.<br>"+
                 "So each time you add a new photo to your cart, "+
                 "you will start with an empty form.", 
        onOk: function () {
          this.clearDefaultSelection(true);   
        },
        okLabel: this.lz('Clear'),
      });
      return;
    }
    
    delete app.buyPresets[this.model.event.id];
    delete app.storage['buyPreset'+ this.model.event.id];
    this.clearForm();
  },
  
  clearForm: function () {
    var i, j, option, addons, product;
    for (i = 0; i < this.optionsView.views.length; i++) {
      option = this.optionsView.views[i];
      option.qty = 0;
      option.active = false;
      if (option.addonsView) {
        ...
        for (j = 0; j < addons.length; j++) {
          ...
          if (addons[j].model.bulkPrice) continue;
          if (addons[j].model.bulkType) continue;
          addons[j].active = false;
          addons[j].selected_id = 0;
          addons[j].updatePrice();
        }
      }
      option.updatePrice();
  ...
  checkForDefaultSelection: function () {
    var match = this.matchesDefaultSelection();
    this.isDefaultSelection = match;
  },
  
  // check to see if current selection matches the saved default one
  matchesDefaultSelection: function () {
    var preset = app.buyPresets[this.model.event.id];
    if (!preset) return false;
    
    var i, j, option, addons, addonPreset, sizePreset, product, active;
    for (i = 0; i < this.optionsView.views.length; i++) {
      option = this.optionsView.views[i];
      sizePreset = preset.main[option.model.size.id+'-'+option.model.color_id];
      if (option.active !== !!sizePreset) return false;
      if (!option.active) continue;
      
      if (option.qty !== sizePreset.qty) return false;
      ...
      if (option.addonsView) {
        ...
        for (j = 0; j < addons.length; j++) {
          ...
          if (addons[j].model.bulkType) continue;
          addonPreset = sizePreset.addons[addons[j].model.id];
          active = addons[j].active || !!addons[j].selected_id;
          if (!active && !addonPreset) continue;
          if (!!addonPreset !== active) return false;
          
          if (addons[j].model.options.length <= 1) {
            if (!!addonPreset !== addons[j].active) return false;
          }
          else {
            if (addonPreset !== addons[j].selected_id) return false;
          }
        }
      }
    }
    ...
    for (i = 0; i < this.productsView.views.length; i++) {
      ...
      for (j = 0; j < product.options.length; j++) {
        ...
        sizePreset = preset.products[product.model.cart_item_id +'-'+ 
                                     option.model.id +'-'+ option.colorId];
        if (!!sizePreset !== option.active) return false;
        if (!sizePreset) continue;
        if (sizePreset.qty !== option.qty) return false;
      }
    }
  ...
  removeFromCart: function (confirmed) {
    if (confirmed !== true) {
      this.confirm({
        message: "This will remove the photo from cart "+
                 "and all products it has been added to.", 
        onOk: function () {
          this.removeFromCart(true);   
        },
        okLabel: this.lz('Remove'),
      });
      return;
    }
    // if (!confirm("This will remove the photo from cart and all products "+
    //     "it has been added to.")) return;
      
    var cart = this.model.event.cart;
    var product, photos;
    
    var cartPhotos = cart.photos.where({
      pid: this.model.id,
    });
    cart.photos.remove(cartPhotos);
    
    // var cartPhoto = cart.photos.get(this.model.id, 'pid');
    // cart.photos.remove([cartPhoto]);
    
    for (var i = 0; i < cart.products.length; i++) {
      product = cart.products.at(i);
      photos = product.photos.where({
        pid: this.model.id,
      });
      cart.products.at(i).photos.remove(photos);
    }
  ...
  save: function () {
    ...
    var preset = { // for saving as default set 
      main: {},
      products: {},
    };
    
    var catId = this.model._cat_id || this.model.cat_id;
    ...
    for (i = 0; i < this.optionsView.views.length; i++) {
      ...
      cartOption = {
        ...
        crop: option.crop && option.crop.getValues(),
      };
      
      preset.main[option.model.size.id +'-'+ option.model.color_id] = {
        qty: option.qty,
        addons: {},
      };
    ...
    if (newPhotos.length && !this.empty) {
      ...
      for (j = 0; j < productView.options.length; j++) {
        ...
        if (option.active) {
          ...
          productPhotos[option.colorId].options.push({
            ...
            // crop: option.cropModel && option.cropModel.changed 
            //       ? option.cropModel.toJSON() : {},
          });
          
          if (option.crop) {
            console.log(option.model.size.id, option.cid, option.crop.cid, option.crop.changed, option.crop.getValues());
          }
          
          // console.log(productPhotos[option.colorId].options[productPhotos[option.colorId].options.length - 1].crop);
          
          preset.products[product.cart_item_id +'-'+ 
                          option.model.id +'-'+ option.colorId] = {
            qty: option.qty,                
          };
        }
    ...
    if (this.saveAsDefault) {
      app.buyPresets[this.model.event.id] = preset;
      app.storage['buyPreset'+ this.model.event.id] = JSON.stringify(preset);
    }
    
    this.parent.model.in_cart = true;
