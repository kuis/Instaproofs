...
module.exports = BaseView.extend({
  ...
  bindings: {
    ...
    basePrice: {
      type: function (el, val) {
        if (this.active) el.innerHTML = this.model.formatMoney(val);
        else el.innerHTML = this.model.formatMoney(this.singleBasePrice);
      },
    ...
    singlePrice: {
      type: function (el, val) {
        ...
        if (!this.active) {
          this.query('.head .discounted-price').innerHTML = 
    ...
    singleBasePrice: {
      type: function (el, val) {
        el.innerHTML = this.model.formatMoney(val);
        if (!this.active) {
          this.query('.head .original-price').innerHTML = 
              this.model.formatMoney(val);
        }
      },
  ...
  render: function () {
    ...
    if (this.parent && this.parent.collection || this.isProductSize) {
      ...
      if (!this.model.size.no_qty) {
        var capacity = 0;
        if (this.cartProduct && this.model.id) {
          capacity = this.cartProduct.capacity[this.model.id];
        }
