...
module.exports = BaseView.extend({
  ...
  props: {
    ...
    // when currently selected variant has failed to load
    failed: ['boolean', true, false],

    // number of times image requests have failed 
    failures: {
      type: 'object',
  ...
  bindings: {
    ...
    'model.current_color_id': {
      type: function () {
        var color = app.colors.get(this.model.current_color_id);
        this.currentVariant = color ? color.prefix : 'original';
        this.updateFailed();
      },
    }
  },
  ...
  initialize: function () {
    ...
    this.cacheElements({
      ...
      img: '.img',
      // imgEl: 'img',
      imgElOriginal: 'img.original',
      imgElBlack: 'img.black',
      imgElSepia: 'img.sepia',
      actionsEl: '.actions',
    ...
    this.imgElOriginal.addEventListener('load', this, false);
    this.imgElOriginal.addEventListener('error', this, false);
    this.imgElBlack.addEventListener('load', this, false);
    this.imgElBlack.addEventListener('error', this, false);
    this.imgElSepia.addEventListener('load', this, false);
    this.imgElSepia.addEventListener('error', this, false);
    ...
    this.listenTo(app, 'reload-images', this.reloadImages);
  },

  reloadImages: function () {
    if (this.model.id == 378920462) console.log('reload-images');
    if (this.failures.original >= 2) {
      this.failures.original = 0;
      this.reloadImage('original');
    }
    if (this.failures.black >= 2) {
      this.failures.black = 0;
      this.reloadImage('black');
    }
    if (this.failures.sepia >= 2) {
      this.failures.sepia = 0;
      this.reloadImage('sepia');
    }
  },
  reloadImage: function (variant) {
    // this.failures[variant] = 0;
    if (this.model.id == 378920462) {
      console.log('reloadImage', variant);
    }
    var img = this.imgElOriginal;
    if (variant === 'black') img = this.imgElBlack;
    else if (variant === 'sepia') img = this.imgElSepia;
  ...
  events: {
    ...
    'click .button.reload': 'reloadImages',
  ...
  updateFailed: function () {
    var variant = 'black';
    if (this.currentVariant === 'original') variant = 'original';
    if (this.currentVariant === 'sepia') variant = 'sepia';
    if (variant === 'original' && app.loadedImages[this.model.thumb_url]) {
      this.failed = false;
    }
    else this.failed = this.failures[variant] >= 2;
  },
  ...
  handleEvent: function (e) {
    switch (e.type) {
      ...
      case 'load':
        ...
        if (e.target && e.target.classList.contains('black')) variant = 'black';
        else if (e.target && e.target.classList.contains('sepia')) {
          variant = 'sepia';
        }
        this.failures[variant] = 0;
        if (variant === 'original') {
      ...
      case 'error':
        ...
        this.failures[variant]++;
        if (this.failures[variant] < 2) {
          this.reloadImage(variant);
        }
        else {
          this.trigger('error');
        }
        this.updateFailed();
        this.model.event.checkServerStatus();
        break;
    }
  },
  ...
  resize: function (ops) {
    ...
    if (!ops.notReally) {
      ...
      this.setImgWidth(this.imageWidth +'px');
      this.setImgHeight(this.imageHeight +'px');
  ...
  setImgTop: function (v) {
    this.imgElOriginal.style.top = v;
    this.imgElBlack.style.top = v;
    this.imgElSepia.style.top = v;
  },
  setImgLeft: function (v) {
    this.imgElOriginal.style.left = v;
    this.imgElBlack.style.left = v;
    this.imgElSepia.style.left = v;
  },
  setImgWidth: function (v) {
    this.imgElOriginal.style.width = v;
    this.imgElBlack.style.width = v;
    this.imgElSepia.style.width = v;
  },
  setImgHeight: function (v) {
    this.imgElOriginal.style.height = v;
    this.imgElBlack.style.height = v;
    this.imgElSepia.style.height = v;
  },
  
  remove: function () {
    BaseView.prototype.remove.apply(this);
    this.imgElOriginal.removeAttribute('src');
    this.imgElBlack.removeAttribute('src');
    this.imgElSepia.removeAttribute('src');
    this.imgElOriginal.removeEventListener('load', this, false);
    this.imgElOriginal.removeEventListener('error', this, false);
    this.imgElBlack.removeEventListener('load', this, false);
    this.imgElBlack.removeEventListener('error', this, false);
    this.imgElSepia.removeEventListener('load', this, false);
    this.imgElSepia.removeEventListener('error', this, false);
    this.el.innerHTML = '';
    this.stopListening();
  ...
  showBuyForm: function () {
    ...
    if (!app.state.loading) {
      ...
      app.view.el.classList.add('loading');
      setTimeout(function () {
        self.showBuyForm();
      });
    ...
    app.view.el.classList.remove('loading');
  ...
  toggleFav: function () {
    ...
    this.model.event.trigger('fav-toggle', this.model.id, catId);
    
    if (!this.model.in_favorites) {
      if (this.parent.collection === this.model.event.favorites.photos) {
  ...
  toggleHide: function () {
    var catId = this.model._cat_id || this.model.cat_id;
    this.model.event.trigger('hide-toggle', this.model.id, catId);
  ...
  onImageTransitionEnd: function () {
    this.imagesEl.classList[this.parent.buyFormDisplayed ? 'add' : 'remove'](
        'pushed-right');
    if (!this.parent.buyFormDisplayed && this.buyForm) {
      this.buyForm.el.classList.add('hidden');
    }
